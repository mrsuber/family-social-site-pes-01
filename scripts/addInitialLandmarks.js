...
async function addInitialLandmarks() {
  try {
    await sequelize.authenticate();
    console.log('✅ Database connected\n');

    // Find the High Commander - Personal Goals General
    const general = await General.findOne({
      where: { name: 'High Commander - Personal Goals' }
    });

    if (!general) {
      console.error('❌ High Commander - Personal Goals General not found. Please create it first.');
      process.exit(1);
    }

    console.log(`📍 Creating landmarks for General: ${general.name}\n`);
    ...
    const landmarks = [
      ...
      },
      {
        ...
        generalId: general.id,
        startDate: new Date('2026-03-20T00:00:00Z'), // This Thursday
        endDate: new Date('2026-03-27T23:59:59Z'), // Next Thursday
        amount: 14,
        currency: 'USD',
        paymentStatus: 'pending',
        amountPaid: 0,
    ...
    console.log('\n═══════════════════════════════════════════════════════════════');
    console.log('✨ ALL LANDMARKS CREATED SUCCESSFULLY! ✨');
    console.log('═══════════════════════════════════════════════════════════════');
